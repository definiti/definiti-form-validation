...
import definiti.tsmodel.TypescriptGenerator
...
object Boot extends App {
  val source = Paths.get("src", "main", "resources", "samples", "Adherent.def")
  val destination = Paths.get("client", "src", "result.ts")
    ...
    parseDefinitiFile(source.toString) match {
      ...
      case Right(ast) =>
        ...
        ASTValidation.validate(ast) match {
          case Valid =>
            val modelSource = TypescriptGenerator.generate(ast)
            val formValidationSource = FormValidationGenerator.generate(ast)
            val source = modelSource + "\n\n" + formValidationSource
            Files.createDirectories(destination.getParent)
            Files.write(destination, Seq(source).asJava, StandardCharsets.UTF_8, StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING)
          case Invalid(errors) =>
